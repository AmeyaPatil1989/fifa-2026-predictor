...
if page == "📅 Today's Matches":
    ...
    # ── Get client's local date via JS → query param ──────────────────────────
    # Inject JS to read the viewer's local date (not server UTC) and push it
    # into the URL as ?local_date=YYYY-MM-DD. On the next rerun Streamlit picks
    # it up. This means the first load uses the fallback logic (ET-based), and
    # every subsequent load uses the real local date.
    ...
    # Use client local date if available, else fall back to ET-based logic
    _client_date = None
    ...
    try:
        ...
        if _client_date:
            ...
            # Check if today (ET) has any pre matches — if so, show today not yesterday
